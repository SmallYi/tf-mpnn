...
def load_one_file(fn, num_clusters=100): #change clusters 10 --> 100
    Xs = []
    ys = []
    ys_cand = []
    dms = []
...
def load_dataset_ttbar(datapath):
    tfr_files = glob.glob(datapath + "/tfr3/{}/*.tfrecords".format(args.target)) #changed tfr2 -> tfr3
    dataset = tf.data.TFRecordDataset(tfr_files).map(_parse_tfr_element, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    return dataset
...
if __name__ == "__main__":
    ...
    batch_size = 10 #changed this from 50 to 10
...
#         KX
    ...
    with strategy.scope():
        ...
        if args.load:
            ...
            print('type of ds_train is {}'.format(type(ds_train)))
